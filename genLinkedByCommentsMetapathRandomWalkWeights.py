...
def printRWWeightsLinkedByComments(ownerUserIdPostsInput, postTagWeightsInput, edgesLinkedByComments, rwWeightsLinkedByCommentsOutput):
	...
	edgesLinkedByCommentsFile = open(edgesLinkedByComments, 'r')
	rwWeightsLinkedByCommentsOutputFile = open(rwWeightsLinkedByCommentsOutput, 'w')

	for line in edgesLinkedByCommentsFile:
		lineParts = line.strip('\n').strip().split(' ')
		user1 = lineParts[0]
		user2 = lineParts[1]
					...
					if postId1 in postTagWeights.keys() and postId2 in postTagWeights.keys():
						for tag1,weight1 in postTagWeights[postId1].items():
							for tag2,weight2 in postTagWeights[postId2].items():
								rwWeightsLinkedByCommentsOutputFile.write( \
									str(postId1) + ' ' + \
									str(postId2) + ' ' + \
									str(tag1) + ' ' + \
									str(tag2) + ' ' + \
	...
	edgesLinkedByCommentsFile.close()						
	rwWeightsLinkedByCommentsOutputFile.close()


def main():
	printRWWeightsLinkedByComments(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
