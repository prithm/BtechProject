...
def printRWWeightsLinkedByCommentsUserMentions(ownerUserIdPostsInput, postTagWeightsInput, edgesLinkedByCommentsUserMentions, rwWeightsLinkedByCommentsUserMentionsOutput):
	...
	edgesLinkedByCommentsUserMentionsFile = open(edgesLinkedByCommentsUserMentions, 'r')
	rwWeightsLinkedByCommentsUserMentionsOutputFile = open(rwWeightsLinkedByCommentsUserMentionsOutput, 'w')

	for line in edgesLinkedByCommentsUserMentionsFile:
		lineParts = line.strip('\n').strip().split(' ')
		user1 = lineParts[0]
		user2 = lineParts[1]
					...
					if postId1 in postTagWeights.keys() and postId2 in postTagWeights.keys():
						for tag1,weight1 in postTagWeights[postId1].items():
							for tag2,weight2 in postTagWeights[postId2].items():
								rwWeightsLinkedByCommentsUserMentionsOutputFile.write( \
									str(postId1) + ' ' + \
									str(postId2) + ' ' + \
									str(tag1) + ' ' + \
									str(tag2) + ' ' + \
	...
	edgesLinkedByCommentsUserMentionsFile.close()						
	rwWeightsLinkedByCommentsUserMentionsOutputFile.close()


def main():
	printRWWeightsLinkedByCommentsUserMentions(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
