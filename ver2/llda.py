...
def load_corpus(filename):
	...
	labels = []
	labelmap = dict()
	num_docs = len(everything)
	for i in xrange(0,num_docs):
		for tag in everything[i]['tags']: 
			labelmap[tag] = 1
		corpus.append(everything[i]['body'].split())
		labels.append(everything[i]['tags'])
...
class LLDA:
	...
	def set_corpus(self, labelset, corpus, labels):
		labelset.insert(0, "commonthingyeverywhere")
		...
		self.n_z_t = numpy.zeros((self.K, V), dtype=int) #save this
