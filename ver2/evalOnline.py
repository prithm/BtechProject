...
def main():
	...
	sqError = 0.0
	meanTrue = []
	w = [-506.7759725244813, 9.824416473572133e-182]
				...
				for tag1 in train:
					...
					if tag1 in tagtagRWR:
						if test[0]  in tagtagRWR[tag1]:
							prob += w[0]*tagtagRWR[tag1][test[0]]
					if tag1 in tagtagRWR2:
						if test[0]  in tagtagRWR2[tag1]:
							prob += w[1]*tagtagRWR2[tag1][test[0]]
					trueProb *= (prob + 0.00000000000001)
				
				sqError += (trueProb - maxprob)**2
			
			meanTrue.append(trueProb)

	u = sqError
	mean = np.mean(meanTrue)
	v = 0.0
	for i in meanTrue:
		v += (i - m)**2
