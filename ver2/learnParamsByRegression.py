...
def main():
	...
	with open(sys.argv[3], 'r') as f:
		for line in f:
			...
			if tag2 not in tagtagRWR2[tag1]:
				continue
			if tag1 not in tagtagRWR3:
				tagtagRWR3[tag1] = {}
			if tag2 not in tagtagRWR3[tag1]:
				tagtagRWR3[tag1][tag2] = 0
			tagtagRWR3[tag1][tag2] = prob
	...
	for tag1 in tagtagCoOccur:
		for tag2 in tagtagCoOccur[tag1]:
			train.append([tagtagRWR[tag1][tag2], tagtagRWR2[tag1][tag2], tagtagRWR3[tag1][tag2]])
	...
	pickle.dump( model, open( "LinearRegression_2.pickle", "wb" ) )
