...
if __name__ == '__main__':
	...
	with open(sys.argv[1], 'r') as f:
		for line in f:
			...
			seedTagProb = {}
			if 'predicted_by_StringAndWVTool_Matching' in post.keys():
				for item in post['predicted_by_StringAndWVTool_Matching']:
					seedTagProb[item[0]] = item[1]
			elif 'predicted_by_StringAndFIC_Matching' in post.keys():
				for item in post['predicted_by_StringAndFIC_Matching']:
					seedTagProb[item[0]] = item[1]
			else:
				for item in post['predicted_by_String_Matching']:
					seedTagProb[item[0]] = item[1]
